...
class Account(db.Model):
    ...
    UFI_id = db.Column(
        db.Integer,
        db.ForeignKey(
            'users_financial_institutions.id',
            ondelete='CASCADE'),
        nullable=False)
    ...
    def get_amount_spent_for_account(
            self,
            start: datetime,
            end: datetime,
            plaid_user_type: str) -> float:
        ...
        transactions = plaid_inst.get_Account_transactions_from_Plaid(
            access_token, start, end, self.account_id)
