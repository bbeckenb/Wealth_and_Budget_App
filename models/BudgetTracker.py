...
class BudgetTracker(db.Model):
    ...
    def find_all_scheduled_today(cls) -> list:
        ...
        return cls.query.filter(
            func.date(
                cls.next_notification_date) == date.today()).all()
    ...
    def scheduled_daily_refresh_budgettrackers(cls):
        ...
        for bt in budgettrackers:
            ...
            if today_date.day == 1:
                ...
                amount_spent = bt.account.get_amount_spent_for_account(
                    today_date.replace(day=1), today_date, bt.user.account_type)
    ...
    def update_amount_spent(self, plaid_user_type: str):
        ...
        if today_date.day == 1:
            ...
            amount_spent = self.account.get_amount_spent_for_account(
                today_date.replace(day=1), today_date, plaid_user_type)
    ...
    def update_next_notify_date(self):
        ...
        self.next_notification_date = self.next_notification_date + \
            timedelta(days=self.notification_frequency)
        db.session.add(self)
        db.session.commit()
